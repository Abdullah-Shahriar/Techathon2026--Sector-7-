...
import { Alert, AlertSetting, Device, Esp32Node, LatestDeviceState, Room, type AlertDocument } from "../models/index.js";
...
export async function updateAlertSettings(input: z.infer<typeof updateAlertSettingsSchema>): Promise<unknown[]> {
  ...
  for (const setting of parsed.settings) {
    const filter = setting.id
      ...
      : {
          scope: setting.scope,
          roomId: setting.roomId ?? null,
          deviceId: setting.deviceId ?? null,
          alertType: setting.alertType
        };

    updated.push(await AlertSetting.findOneAndUpdate(
      filter,
      {
        $set: {
          scope: setting.scope,
...
export async function upsertActiveAlert(input: UpsertAlertInput): Promise<AlertDocument | null> {
  ...
  if (existing) {
    ...
    if (shouldRepeat) {
      ...
      await existing.save();
      emitRealtime("alert_created", existing);
    }
    return existing;
  }
  ...
  emitRealtime("alert_created", alert);
  return alert;
}
